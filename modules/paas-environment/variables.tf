...
variable "admin_source_ip" {
  description = "CIDR allowed to reach the client VM over SSH — your current public IP, e.g. \"1.2.3.4/32\"."
  type        = string
}
...
variable "sku_name" {
  description = <<-EOT
    ...
    for this environment. Verify exact names available in the target region
    first: az postgres flexible-server list-skus --location <region>
  EOT
  type        = string
}
